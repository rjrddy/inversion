...
// POST /api/resumes/:id/versions/rollback { targetVersionId }
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const target = await prisma.resumeVersion.findUnique({ where: { id: targetVersionId } })
  if (!target) return NextResponse.json({ error: "Target version not found" }, { status: 404 })

  // Create a new version with same content and branchName to represent rollback
  const rollback = await prisma.resumeVersion.create({
    data: {
      resumeId: id,
